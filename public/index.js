...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.getElementById("forgot-password-link").textContent =
		messages.forgotPassword;
	document.getElementById("resetPassword-text").textContent =
		messages.resetPassword;
	document.getElementById("email-text3").textContent = messages.email;
	document.getElementById("resetPassword-text2").textContent =
		messages.resetPassword;
	...
	.addEventListener("submit", async (event) => {
		...
		try {
			const response = await fetch(
				`${endpointUrl}/api/v1/password-reset`,
				{
					method: "POST",
